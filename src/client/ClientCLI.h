...
private:
    ...
    std::string handleRetr(const std::string& remoteFile, const std::string& localFile);

    // Xử lý lệnh STOR: PASV → mở UDP → RDTSender → gửi file
    std::string handleStor(const std::string& localFile, const std::string& remoteFile,
                           bool unique = false, bool append = false);
