...
uint16_t calculateChecksum(const CustomUDPHeader &hdr, const uint8_t *payload,
                           size_t payloadLen) {
...
bool verifyChecksum(const CustomUDPHeader &hdr, const uint8_t *payload,
                    size_t payloadLen) {
...
void serializeHeader(const CustomUDPHeader &hdr, uint8_t *buffer) {
  ...
  size_t offset = 0;
  std::memcpy(buffer + offset, &netSeq, sizeof(netSeq));
  offset += sizeof(netSeq);
  std::memcpy(buffer + offset, &netAck, sizeof(netAck));
  offset += sizeof(netAck);
  std::memcpy(buffer + offset, &netLen, sizeof(netLen));
  offset += sizeof(netLen);
  std::memcpy(buffer + offset, &netWin, sizeof(netWin));
  offset += sizeof(netWin);
  std::memcpy(buffer + offset, &netCsum, sizeof(netCsum));
  offset += sizeof(netCsum);
...
CustomUDPHeader deserializeHeader(const uint8_t *buffer) {
  ...
  std::memcpy(&netSeq, buffer + offset, sizeof(netSeq));
  offset += sizeof(netSeq);
  std::memcpy(&netAck, buffer + offset, sizeof(netAck));
  offset += sizeof(netAck);
  std::memcpy(&netLen, buffer + offset, sizeof(netLen));
  offset += sizeof(netLen);
  std::memcpy(&netWin, buffer + offset, sizeof(netWin));
  offset += sizeof(netWin);
  std::memcpy(&netCsum, buffer + offset, sizeof(netCsum));
  offset += sizeof(netCsum);
